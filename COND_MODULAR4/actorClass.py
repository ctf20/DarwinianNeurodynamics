...
class actorClass(ALModule):
  ...
  def __init__(self,name = "actor", count = 0, inputs = None, messageInputs = None, function = None, typeA = 0):
    #MAKE RANDOM ACTOR
...
#    print "Messageinput = " + str(m)
    ...
    if self.actorType is 1: #FUNCTION ACTOR
      ...
      if self.functionName is "product":
        #print "Function actor type 1:product"
        functRaw = v[0] + v[1]
    ...
    #STOCHASTIC HILL CLIMBER TO MINIMIZE MESSAGE INPUT
      ...
      for i,v in enumerate(self.parameters):
        self.parameters[i] = self.parameters[i] + 1.0*(random.random()-0.5)
      ...
      self.motion.post.changeAngles(names,angles,0.5)#Actually move the motors according to values in angles list.
      #time.sleep(0.1)
