...
class actorPopulationClass(ALModule):
  ...
  def __init__(self,name, kind, from_file):
    ...
    if from_file is False and kind is 3:
      for i in range(self.MAX_POP_SIZE):
        self.actors[self.totalCount] = actorClass(typeA = "sensory", nameA = "actor",count = self.totalCount, sensors = [141,142,143],messages = [ self.totalCount], motors = None, function = "position", parameters = None)
        self.totalCount = self.totalCount + 1
...
##
      ...
      del_val = -1
      for index, i in enumerate(newMol):
        if random.random() < 0.1 and deleted == 0 and len(newMol) > 3:
