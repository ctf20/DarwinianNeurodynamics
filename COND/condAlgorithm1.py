...
class MotorBabbling(object):
     def __init__(self, motion, memory, sonar, posture):
         ...
         self.codn = codn() #Construct the data sructure for storing the populations etc.. 
     ...
     def runCODN(self):#RUN CODN
          #Initialize the population of actors and games/targets/goals
          self.codn.createPopulations()
          self.runAllActors()
     ...
     def runAllActors(self):#RUN all CODN Actors
          ...
          for i in range(len(self.codn.popActors)):
               if(self.codn.popActors[i].type is 0):
                    #Get sensory input
                    sensedAngles = self.get_sensor_values()
                    #Send sensory input to the actor
...
def main (argv=None):
 ...
 #   print posture.getPostureFamily()
    ...
    #RUN THE CO-EVOLUTIONARY DARWINIAN NEURODYNAMICS ALGORITHM (COD-N)
    mb.runCODN()
