...
#    ALModule.__init__(self,str(uuid.uuid4()))
    ALModule.__init__(self,str(count))
    ...
    if self.type == "motorLSM":#http://briansimulator.org/?utm_source=twitterfeed&utm_medium=twitter
       ...
       '''
       self.P = NeuronGroup(100, eqs, threshold=-50*mV, reset=-60*mV)
       self.P.v = -60*mV+10*mV*rand(len(self.P))
       self.Pe = self.P.subgroup(80)
       self.Pi = self.P.subgroup(20)
...
#       show()
  ...
  def mutate(self):
    ...
    if self.type == "motorP" or self.type == "motorIzh":      
  ...
  def act(self):
    ...
    if self.sensors is not None:
         ...
         for j, i in enumerate(self.sensors):      
    ...
    if self.messages is not None:
      for j,i in enumerate(self.messages):
            ...
            if name not in dictTemp:
              ...
              for ind3, y in enumerate(self.parameters[1][len(v)-1 + index]):
                for ind4, p in enumerate(y):
                  if p in sums.keys(): 
                     tot = tot + sums[p]*self.parameters[2][len(v)-1 + index][ind4]
