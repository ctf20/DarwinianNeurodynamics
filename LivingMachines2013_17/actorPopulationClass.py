...
class actorPopulationClass(ALModule):
  ...
  def __init__(self,name, kind, from_file):
    ...
    if from_file is False and kind is 4:
      for i in range(self.MAX_POP_SIZE):
        self.actors[self.totalCount] = actorClass(copyA = False, atomA = None, typeA = "sensory", nameA = "actor",count = self.totalCount, sensors = [140],messages = [self.totalCount], messageDelays = [1],motors = None, function = "position", parameters = None)
        self.totalCount = self.totalCount + 1
        ...
        self.actors[self.totalCount] = actorClass(copyA = False, atomA = None, typeA = "motorIzh", nameA = "actor",count = self.totalCount,
                                                  ...
                                                  [random.random()-0.5 for x in range(0, 5)]], [1, 1, 1]])
        self.totalCount = self.totalCount + 1
        self.actors[self.totalCount] = actorClass(copyA = False, atomA = None, typeA = "motorP", nameA = "actor",count = self.totalCount, sensors = None, messages = [self.totalCount-2], messageDelays = [2], motors = [self.mm.getRandomMotor(),self.mm.getRandomMotor(),self.mm.getRandomMotor()], function = "sequence", parameters = [[random.randint(0,3)], [2*(random.random()-0.5), 2*(random.random()-0.5), 2*(random.random()-0.5)], [1, 1, 1]])
        self.totalCount = self.totalCount + 1
 ...
 #       print str(mol) + " is the same as "+ str(self.molecules[index])
  ...
  def recordMolecule(self, niche):
    ...
    if mol not in self.molecules:
      ...
      self.moleculeNiche.append(niche)
